...
class SelfAttention(nn.Module):
    def __init__(self, channels, reduction=8):
        ...
        self.query_conv = nn.Conv1d(channels, channels // reduction, kernel_size=1)
        self.key_conv = nn.Conv1d(channels, channels // reduction, kernel_size=1)
    ...
    def forward(self, x):
        ...
        queries = self.query_conv(x).view(batch_size, -1, width)
        keys = self.key_conv(x).view(batch_size, -1, width)
        values = self.value_conv(x).view(batch_size, -1, width)

        # Calculate attention weights
        energy = torch.bmm(queries.transpose(1, 2), keys)
        attention = F.softmax(energy, dim=1)

        # Apply attention to values
        out = torch.bmm(values, attention.transpose(1, 2))
        out = out.view(batch_size, self.channels, width)
