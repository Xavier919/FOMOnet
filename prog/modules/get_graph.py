...
if __name__ == "__main__":
    ...
    for bin_ in bins:
        ...
        fpr, tpr, _ = metrics.roc_curve(target, preds)
        roc_auc = auc(fpr, tpr)
        plt.scatter(fpr, tpr, color = 'grey', s=0.001, alpha=0.005)
    ...
    for bin_ in bins:
        ...
        precision, recall, _ = pr_curve(preds, target)
        auc_pr = auc(recall, precision)
        plt.scatter(recall, precision, color = 'grey', s=0.001, alpha=0.005)
        plt.ylim(0.9, 1.01), plt.xlim(0, 1)
    preds_ = [x['out'] for x in reports.values()]
    target_ = [x['mapped_cds'] for x in reports.values()]
    preds = cat(preds_)
    ...
    precision, recall, _ = pr_curve(preds, target)
    auc_pr = auc(recall, precision)
    plt.plot(recall, precision, color = 'lime', linewidth=0.5)
    plt.ylim(0.95, 1.01), plt.xlim(0, 1)
    plt.xlabel("Recall"), plt.ylabel("Precision"), plt.title('PR curve')
    plt.legend(['PR AUC: {}'.format(round(auc_pr, 3))])
