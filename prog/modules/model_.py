...
class FOMOnet(nn.Module):
    ...
    def __init__(self, num_channels=4):
        super().__init__()
        #encoder
        self.conv_encoder1 = self.enc_block(num_channels, 32)
        self.conv_encoder2 = self.enc_block(32, 64)
        self.res_encoder2 = self.res_block(32, 64)
        self.conv_encoder3 = self.enc_block(64, 128)
        self.res_encoder3 = self.res_block(64, 128)
        self.conv_encoder4 = self.enc_block(128, 256)
        self.res_encoder4 = self.res_block(128, 256)
        self.conv_encoder5 = self.enc_block(256, 512)
        self.res_encoder5 = self.res_block(256, 512)
        self.conv_encoder6 = self.enc_block(512, 1024)
        ...
        #decoder
        self.conv_decoder4 = self.dec_block(1024, 256)
        self.res_decoder4 = self.res_block(1024, 256)
        self.transpose4 = nn.ConvTranspose1d(in_channels=256, out_channels=256, kernel_size=2, stride=2)

        self.conv_decoder3 = self.dec_block(512, 128)
        self.res_decoder3 = self.res_block(512, 128)
        self.transpose3 = nn.ConvTranspose1d(in_channels=128, out_channels=128, kernel_size=2, stride=2)

        self.conv_decoder2 = self.dec_block(256, 64)
        self.res_decoder2 = self.res_block(256, 64)
        self.transpose2 = nn.ConvTranspose1d(in_channels=64, out_channels=64, kernel_size=2, stride=2)

        self.conv_decoder1 = self.dec_block(128, 32)
        self.res_decoder1 = self.res_block(128, 32)
        self.transpose1 = nn.ConvTranspose1d(in_channels=32, out_channels=32, kernel_size=2, stride=2)

        self.final_layer = self.final_block(64, 32, 1)
    ...
    @staticmethod
    def enc_block(in_channels, out_channels, k=5, p=0.5):
    ...
    @staticmethod
    def dec_block(in_channels, out_channels, k=5, p=0.5):
